...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		float x2 = x;
		float x3 = (float) (x+0.5*TRI_SIZE);
		
		float y1 = (float) (y+TRI_SIZE* Math.sqrt(3)/6); 
		float y2 = (float) (y-TRI_SIZE* Math.sqrt(3)/3); 
		float y3 = (float) (y+TRI_SIZE* Math.sqrt(3)/6); 
		...
		text("Land Quake", 65, 165);
		text("Ocean Quake", 65, 205);
		text("Size ~ Magnitude",65, 220);
		
		fill(color(255,255,0,190));
		ellipse(50, 245, 10, 10);
		fill(color(0,100,200,150));
	...
	private void printQuakes() {
		...
		for (Marker cm : countryMarkers) {
			...
			for (Marker qm: quakeMarkers) {
				...
				if (em.isOnLand) {
					String quakeCountry = (String)em.getProperty("country");
					if(name==quakeCountry) {
					counter++;
					}
				}
			}
			if (counter>=1) {System.out.println(name + " : " + counter);}
		}
